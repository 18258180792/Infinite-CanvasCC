...
function promptNodeExpandedHeight(node){
    ...
    const skillExtra = node?.llmEnabled ? 78 : 0;
    return (node?.llmSystemEnabled ? 420 : 360) + skillExtra + smartNodeInputThumbsHeight(promptNodeInputImages(node)) + extra + upstreamExtra + promptNodeSplitExtraHeight(node);
}
...
async function loadCanvas(){
    ...
    try {
        ...
        let recoveredStaleLlmRuns = false;
        nodes.forEach(n => {
            if(n.type === 'smart-minimax') n.timelinePlaying = false;
            // LLM calls are browser-bound and have no server-side task id to resume after a
            // refresh or server restart. A persisted running flag is therefore always stale.
            if(n.type === 'smart-prompt' && n.running){
                n.running = false;
                n.staleRunRecoveredAt = nowMs();
                recoveredStaleLlmRuns = true;
            }
        ...
        applyViewport();
        render();
        if(cleanedDetachedInputs || cleanedCompletedState || recoveredLoopOutputs || hiddenCompletedTimers || recoveredStaleLlmRuns) scheduleSave();
        resumeSmartPendingTasks();
        resumeJimengPendingNodes();
        startCanvasMetaPoll();
...
function promptNodeBodyHtml(node){
    ...
    const llmParams = node.llmEnabled ? `
        <div class="prompt-node-llm">
            ...
            <div class="llm-skill-control-row"><select class="llm-skill-select prompt-llm-skills" multiple size="3" aria-label="选择 Skills">${(window.LLMSkills?.list || []).length ? (window.LLMSkills.list || []).map(skill => `<option value="${escapeHtml(skill.id)}" ${(node.llmSkillIds || []).map(String).includes(String(skill.id)) ? 'selected' : ''}>${escapeHtml(skill.name || skill.id)}</option>`).join('') : '<option disabled>暂无 Skill，请先新增</option>'}</select><button type="button" class="llm-skill-manage prompt-llm-skill-manage">管理</button></div>
...
function bindPromptNodeControls(el, node){
    ...
    const skillEl = el.querySelector('.prompt-llm-skills');
    if(skillEl) skillEl.onchange = e => { e.stopPropagation(); node.llmSkillIds = [...skillEl.selectedOptions].map(option => option.value); scheduleSave(); };
    const skillManageEl = el.querySelector('.prompt-llm-skill-manage');
    if(skillManageEl) skillManageEl.onclick = e => { e.preventDefault(); e.stopPropagation(); LLMSkills.openManager({selectedIds:node.llmSkillIds || [], onChange(ids){ node.llmSkillIds = ids; render(); scheduleSave(); }}); };
    ...
    try {
        ...
        const result = await fetch('/api/canvas-llm', {
            ...
            body:JSON.stringify({
                ...
                skill_ids:Array.isArray(node.llmSkillIds) ? node.llmSkillIds : []
            })
        }).then(async r => {
            if(!r.ok) throw new Error(await r.text());
            return r.json();
        ...
        const message = e?.name === 'AbortError' ? 'LLM 请求超时，请检查模型服务后重试' : (e.message || tr('smart.promptLlmFailed'));
...
window.onload = async () => {
    ...
    await loadConfig();
    try { await LLMSkills.load(); } catch(e) {}
    await loadAssetLibrary();
    await loadCanvas();
    syncApiKindToggleVisibility();
    render();
